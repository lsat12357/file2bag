...
module MappingMethods
  module Cleanup
    ...
    def ormaps_cleanup(collection, graph, subject)
      eastwest = graph.query([subject, @namespaces['exif']['gpsLongitude'], nil]).to_a
      unless eastwest.empty?
        west_point = eastwest.first.object.to_s.gsub!("\"","")
        east_point = eastwest.last.object.to_s.gsub!("\"","")
        northsouth = graph.query([subject, @namespaces['exif']['gpsLatitude'], nil]).to_a
        north_point = northsouth.first.object.to_s.gsub!("\"","")
        south_point = northsouth.last.object.to_s.gsub!("\"","")
        graph.delete([subject, @namespaces['exif']['gpsLongitude'], nil])
        graph.delete([subject, @namespaces['exif']['gpsLatitude'], nil])
        graph << RDF::Statement.new(subject, @namespaces['exif']['gpsLongitude'], "#{east_point}, #{west_point}")
        graph << RDF::Statement.new(subject, @namespaces['exif']['gpsLatitude'], "#{north_point}, #{south_point}")
        graph
      end
    end
    ...
    def load_compound_objects(collection, graph, subject)
      begin
        ...
        #cpd_url = "http://oregondigital.org/cgi-bin/showfile.exe?CISOROOT=/#{collection}&CISOPTR=#{cis_id}"
        ...
        cpd_fname = results.first[:full].to_s
        puts "fname: " + cpd_fname
        if cpd_fname.include? ".cpd"
            cpd_file = File.open(@opts['CONTENTDIR'] + '/' + cpd_fname)
